...
def prepare_batch(batch_examples):
    # batch_examples is a list of dicts
    audio_arrays = [ex["audio"]["array"] for ex in batch_examples]
    inputs = processor(
        audio_arrays,
        sampling_rate=16000,
        return_tensors="np",
    ...
    labels = [tokenizer(ex["transcription"]).input_ids for ex in batch_examples]
